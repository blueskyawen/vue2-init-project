...
(function () {
    ...
    var generateTitle = function (tag, title) {
      var openTag = '<' + tag + ' contenteditable="false" draggable="false">';
      var closeTag = '</' + tag + '>';
    ...
    var generateTocHtml = function (editor) {
      ...
      return '<div class="' + editor.dom.encode(getTocClass(editor)) + '" contenteditable="false"  draggable="false">' + html + '</div>';
    };
    var generateTocContentHtml = function (editor) {
      ...
      for (i = 0; i < headers.length; i++) {
        ...
        html += '<a href="#' + h.id + '" title="' + h.title +'">' + h.title + '</a>';
    ...
    var setup = function (editor) {
      ...
      editor.on('SetContent', function () {
        ...
        if ($tocElm.length) {
          ...
          $tocElm.children(':first-child').attr('contentEditable', false);
        }
      });
    };
